...
public enum GearSlot
    ...
    public class Gear
    {
        ...
        public void Equip(Item item, Player player)
        {
            ...
            {
                case Armor:
                    if (!Checks.ArmorTypeCheck(player.CharacterInfo[1].ArmorType, item.ArmorType)) return;
                    break;
                case Weapon:
                    if (!Checks.WeaponStyleCheck(player.CharacterInfo[1].WeaponStyle, item.ArmorType)) return;
                    break;
                default:
                    return;
            }

            if (!Checks.LevelCheck(player.Skills.Lookup("PLAYER").Level, item.RequiredLevel)) return;
